...
2. 用系统浏览器打开 `http://192.168.2.17:8080/ai` 登录页
3. 等待用户完成账密登录（最长 3 分钟）

从脚本输出中提取 `LOGIN_URL:` 开头的那一行，获取完整登录链接。

- 若脚本日志显示"已自动打开建必优登录页"，告知用户："✅ 已自动打开建必优登录页，请在浏览器中完成账号密码登录..."
- 否则（企业微信等环境无法自动弹出浏览器），直接把登录链接发给用户，告知："请点击以下链接在浏览器中完成登录：\n{LOGIN_URL}"
